...
class JsonReportParserTest {
    private final IReportParser reportParser = new JsonReportParser();

    @ParameterizedTest
    @ValueSource(strings = {
            "D:/Telran/Homework/Java/SummaryTask2023_08_15/src/test/resources/validReport.json",
            "src/test/resources/validReport.json"
    })
    void testParseReportWithValidFilePathAndReportFormat(String filePath) {
        assertDoesNotThrow(() -> {
            List<Integer> expected = List.of(1, 3, 4, 11, 123);
    ...
    @Test
    void testParseReportThrowsExceptionWhenReportIsBlank() {
        assertThrows(IllegalArgumentException.class, () -> reportParser.parseReport("src/test/resources/emptyReport.txt"));
    }

    @Test
    void testParseReportThrowsExceptionWhenReportIsEmpty() {
    assertThrows(EmptyReportFileException.class, () -> reportParser.parseReport("src/test/resources/emptyReport.json"));
    }
}